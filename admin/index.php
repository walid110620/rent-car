...
if(isset($_REQUEST['nom'],$_REQUEST['password'])){
  ...
  $req="SELECT * FROM `admin` WHERE `emailA`='$nom' and `passwordA`='".sha1($pass)."'";
  $result=@mysql_query($req) or die(@mysql_error());
  $sql=@mysql_fetch_assoc($result);
  if($sql){
    header('location: admin.php ')or die(@mysql_error());
  }else{
    $message = "Le nom d'utilisateur ou le mot de passe est incorrect.";
  }
...
?>
    ...
    <a href="../index.php"> <span class="fas fa-times" id="close-login-form"></span></a> 
      <form  action="" method="POST" >
        ...
        <p>don't have an account <a href="allAdmin.php"> create one</a></p>
